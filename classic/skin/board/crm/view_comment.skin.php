					...
					<?php if($list[$i]['is_reply'] || $list[$i]['is_edit'] || $list[$i]['is_del'] || $is_shingo || $is_admin) {
					 ...
					 ?>
						<div class="print-hide pull-right font-11 ">
							...
							<?php } ?>
                            <?php
                            if ($list[$i]['is_del'] || $is_app_admin && $list[$i]['mb_id'] != "delete" && $cmt['mb_id'] != "delete")  { ?>
                                <a onclick="deleteComment('<?php echo $bo_table; ?>', '<?php echo $comment_id; ?>');">
