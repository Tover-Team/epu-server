...
// 반응형 - 기본 3개 나열
$item = (isset($boset['item']) && $boset['item'] > 0) ? (int)$boset['item'] : 4;
...
// 글내용 - 기본 100자
$is_cont = (isset($boset['lcont']) && $boset['lcont'] > 0) ? (int)$boset['lcont'] : 0;

// 줄수 - 기본 4줄
$is_line = (isset($boset['cline']) && $boset['cline'] >= 1) ? (int)$boset['cline'] : 2;
...
$fa_photo = (isset($boset['ficon']) && $boset['ficon']) ? apms_fa($boset['ficon']) : '<i class="fa fa-cutlery"></i>';
...
$is_good = (isset($boset['lgood']) && $boset['lgood']) ? false : true;

// 보임설정
$is_hit = (isset($boset['lhit']) && $boset['lhit']) ? true : false;
...
<div class="" style="margin-top:10px;clear:both;height:60px;background:white;width:100%;border:1px solid #c3c3c3;margin-bottom:5px;padding:6px;line-height: 54px">
    <p style="font-size:24px;">Vote</p><br>
</div>
<div class="" style="margin-top:10px;clear:both;height:65px;background:white;width:100%;margin-bottom:5px;line-height: 65px;">



	<div class="" style="margin-left:1%;width:48%;float:left;text-align: center;background: #1dc116;color: #fff">
        <a style="color: #fff" href="<?php echo $write_href ?>">투표 등록하기</a>
	</div>
    <div class="" style="margin-left:2%;width:48%;float:left;text-align: center;background: #1dc116;color: #fff">
        <a style="color: #fff" href="<?php echo $write_href."_group" ?>">그룹투표 등록하기</a>
    </div>
		...
		<form name="fboardlist" id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post" role="form" class="form">
			...
			<div class="list-board">
				...
				for ($i=0; $i < $list_cnt; $i++) { 
					...
					// 글정보
					$wr_data = apms_unpack($list[$i]['wr_content']);

					// 별점
					$vstar = apms_post_star($list[$i], 'fa-lg '.$vstar_color);
									...
									<div class="list-text">
										...
										<div class="list-desc">
											<a href="<?php echo $list[$i]['href'];?>"<?php echo $list[$i]['target'];?><?php echo $is_modal_js;?>>
												<strong class="en">투표: <?php echo $list[$i]['wr_subject'];?></strong>
												<?php if($is_cont) { ?>
													<div class="h5"></div>
													<div class="text-muted font-12">
														<?php echo apms_cut_text($wr_data['content'], $is_cont); ?>
													</div>
												<?php } ?>
											</a>
										</div>
										<div class="list-info font-14 en">
                                            <?php
                                            $now = date("Y-m-d");
                                            $end_date = $list[$i]['wr_endDate'];
...
//                                                echo "비교할 시간이 현재시간보다 작습니다.";
                                            ...
                                            ?>
											<div class="pull-left" title="평균 <?php echo sprintf("%2.1f", $vstar['score']);?> / <?php echo $vstar['cnt'];?>명 참여">
                                                <?php echo "상태: "?><?php echo $list[$i]['wr_2'] == "1" && $str_now <= $str_target ? "진행" : "종료";?>
<!--												--><?php //echo $vstar['star'];?>
											</div>
			...
			<?php if ($is_checkbox || $setup_href || $admin_href) { ?>
				<div class="list-admin">
					<div class="btn-group" role="group">
						<?php if ($is_checkbox) { ?>
							<button type="button" id="btn_chkall" class="btn btn-<?php echo $btn1;?> btn-sm"><i class="fa fa-check"></i><span class="hidden-xs"> 전체선택</span></button>
