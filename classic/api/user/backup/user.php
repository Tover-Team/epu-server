...
class User {
    ...
    function updatePush($mb_id, $push_token, $store_type, $is_push_receive) {
        ...
        if ($result == null) {
            ...
            if ($result == null) {
                $this->api->setCode(RPC_QUERY_ERROR);
                $this->api->setMessage("수정 후 반환에 실패했습니다.");
                return;
            }
            $this->api->setCode(RPC_SUCCESS);
            $this->api->setMessage("수정에 성공하였습니다.");
...
//                LIMIT $offset, $limit";
        ...
        if ($keywords == "") {
            $sql = "SELECT * 
                FROM g5_member
                WHERE mb_id != 'admin' AND mb_id NOT LIKE '%tester%'
                ORDER BY mb_name";
        } else {
            $sql = "SELECT * 
                FROM g5_member
                WHERE mb_id != 'admin' AND mb_id NOT LIKE '%tester%' AND mb_id LIKE '%{$keywords}%' OR mb_nick LIKE '%{$keywords}%' OR mb_name LIKE '%{$keywords}%'  
                ORDER BY mb_name";
        }
