...
class Vote
{
    ...
    function vote($id, $mb_id, $no)
    {
        ...
        if ($mb['mb_10'] == '1') {
            ...
            $this->api->setMessage("투표할 수 없는 유저입니다.");
            return $this->api;
        }
    ...
    function unVote($id, $mb_id)
    {
        if (true) {
            $this->api->setCode(RPC_SUCCESS);
            $this->api->setMessage("한번 투표하면 취소할 수 없습니다.");
            return $this->api;
        }
        // 종료 여부 판단
        $row = $this->_getInfo($id);
    ...
    function getSubVote()
    {
        ...
        if ($keywords == "") {
            $sql = "SELECT * 
                FROM g5_write_vote
                WHERE wr_is_comment = 0 AND is_sub = true AND wr_parent = '{$wr_id}'
                ORDER BY wr_id
                DESC ";
        } else {
            $sql = "SELECT * 
                FROM g5_write_vote
                WHERE wr_is_comment = 0 AND is_sub = true AND wr_parent = '{$wr_id}' AND mb_id like '%{$keywords}%' 
                ORDER BY wr_id
                DESC ";
        }
    ...
    function getList($requester, $table, $offset, $limit)
    {
        if (!isset($offset)) {
            $offset = 0;
        }
...
//        }
        ...
        if ($keywords == "") {
            $sql = "SELECT * 
                ...
                WHERE wr_is_comment = 0 AND is_sub = false ";
        } else {
            $sql = "SELECT * 
                FROM g5_write_$table
                WHERE wr_is_comment = 0 AND is_sub = false AND mb_id like '%{$keywords}%' ";
        }
        ...
        // 아직 시작안한 투표는 안보이도록
        // 오늘 일자와 종료 일자를 비교
        $current_date = date('Y-m-d\TH:i:s', time());
        $sql = $sql .  "AND wr_startDate < '$current_date' AND '$current_date' <= wr_endDate ";
        ...
        if ($requester) {
            ...
            switch ($mb['mb_grade']) {
                ...
                // 대위원
                case "2":
                    $sql = $sql . "AND (wr_1 = '1' OR wr_1 = '3' OR wr_1 = '4') ";
                    break;
                // 집행부
                case "3":
                    $sql = $sql .  "AND (wr_1 = '1' OR wr_1 = '2' OR wr_1 = '4') ";
                    break;
                // 임시외부자
                case "6":
                    $sql = $sql .  "AND wr_1 = '6' ";
    ...
    function start()
    {
        ...
        $vote = $this->_getInfo($wr_id);
        if (!$vote) {
            $this->api->setCode(RPC_PARAM_ERROR);
            $this->api->setMessage("투표 정보를 얻어오는데 실패했습니다.");
            return $this->api;
        }

        $result = $this->_startVote($wr_id, "1");
        if ($result == null) {
            $this->api->setCode(RPC_QUERY_ERROR);
        ...
        $this->app->push_message("투표 알림", "새로운 투표가 게시되었습니다.", "", "");
    ...
    function stop()
    {
        ...
        if ($vote['is_sub'] == true) {
            $this->api->setCode(RPC_PARAM_ERROR);
            $this->api->setMessage("서브 투표는 중지할 수 없습니다.");
            return $this->api;
        }
        if ($vote['is_group'] == true) {
            $sql = "SELECT * FROM g5_write_vote WHERE wr_is_comment = 0 AND is_sub = true AND wr_parent = '{$wr_id}' ORDER BY wr_id DESC";
            $result = $this->db->queryListUnmarshal($sql);
            while ($row = mysqli_fetch_assoc($result)) {
                $w = $this->_updateVote($row['wr_id'], "-1");
                if ($w == null) {
                    $this->api->setCode(RPC_QUERY_ERROR);
                    $this->api->setMessage("서브투표 중지 중 에러가 발생했습니다.");
                    $this->api->setEntity(false);
                    return;
                }
            }
    ...
    function addSubVote()
    {
        ...
        // 그룹 속성을 그대로 가져옴
        $parent = $this->_getInfo($wr_parent);
        if ($parent == null) {
            $this->api->setCode(RPC_PARAM_ERROR);
            $this->api->setMessage("널입니다." + $wr_parent);
            $this->api->setEntity("");
            return $this->api;
        }

        $wr_content = $parent["wr_content"];
        $wr_startDate = $parent["wr_startDate"];
        $wr_endDate = $parent["wr_endDate"];
        $mb_id = $parent["mb_id"];
        $wr_name = $parent["wr_name"];
        $wr_1 = $parent["wr_1"];
        $wr_2 = "1";
        $current_date = date('Y-m-d H:i:s', time());
