...
include_once('./push_notifiaction.php');
...
$memID = $_REQUEST["app_user_id"];
if ($memID) {
    $member = get_member($memID);
    $is_guest = false;
...
$wr_startDate = '';
if (isset($_POST['wr_startDate'])) {
    $wr_startDate = substr($_POST['wr_startDate'],0,1000);
    $wr_startDate = trim(strip_tags($wr_startDate));
    $wr_startDate = preg_replace("#[\\\]+$#", "", $wr_startDate);
}

$wr_endDate = '';
if (isset($_POST['wr_endDate'])) {
    $wr_endDate = substr($_POST['wr_endDate'],0,1000);
    $wr_endDate = trim(strip_tags($wr_endDate));
    $wr_endDate = preg_replace("#[\\\]+$#", "", $wr_endDate);
}

$is_group = '';
if (isset($_POST['is_group'])) {
    $is_group = substr($_POST['is_group'],0,1000);
    $is_group = trim(strip_tags($is_group));
    $is_group = preg_replace("#[\\\]+$#", "", $is_group);
}
$is_group = $is_group == "Y" ? true : false;

$is_sub = '';
if (isset($_POST['is_sub'])) {
    $is_sub = substr($_POST['is_sub'],0,1000);
    $is_sub = trim(strip_tags($is_sub));
    $is_sub = preg_replace("#[\\\]+$#", "", $is_sub);
}
$is_sub = $is_sub == "Y" ? true : false;

$is_anonymous = '';
if (isset($_POST['is_anonymous'])) {
    $is_anonymous = substr($_POST['is_anonymous'],0,1000);
    $is_anonymous = trim(strip_tags($is_anonymous));
    $is_anonymous = preg_replace("#[\\\]+$#", "", $is_anonymous);
}

$is_secret = '';
if (isset($_POST['is_secret'])) {
    $is_secret = substr($_POST['is_secret'],0,1000);
    $is_secret = trim(strip_tags($is_secret));
    $is_secret = preg_replace("#[\\\]+$#", "", $is_secret);
}
...
//        alert("게시물 작성에 실패했습니다.");
    ...
    if ($bo_table != "schedule" && $is_group == false) {
        $push_type = _getPushType($bo_table);
        $message = "{$bo_table}게시판에 새글이 작성됐습니다.";
        ...
        $result = sql_query($sql);
        if ($result == null) {
            alert('알림 발송에 실패하였습니다.');
            exit;
        }
        push_message("새글 알림", $message, "", "");
    }
...
if ($file_upload_msg)
    ...
    if ($memID) {
        echo "<script> document.location.href='http://epu8541.cafe24.com/endapp.html'; </script>";
    } else {
